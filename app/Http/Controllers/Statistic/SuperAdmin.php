...
class SuperAdmin extends Controller
{
    ...
    {
        $period = $request->input('period', 'weekly'); // Default to monthly
        ...
        $data = [
            'total_organization' => User::where('role', 'organization')
            ...
            'total_asset'        => Asset::whereBetween('created_at', [$startOfPeriod, $endOfPeriod])->count(),

            'total_ticket'       => Ticket::whereBetween('created_at', [$startOfPeriod, $endOfPeriod])->count(),

            'total_inspections'  => InspectionSheet::whereBetween('created_at', [$startOfPeriod, $endOfPeriod])->count(),

            'total_card'         => JobCard::whereBetween('created_at', [$startOfPeriod, $endOfPeriod])->count(),
        ];
    ...
    //chart superadmin
    public function chartsuperAdmin(Request $request)
    {
